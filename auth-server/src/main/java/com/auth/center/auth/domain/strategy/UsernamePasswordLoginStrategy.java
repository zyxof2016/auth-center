...
public class UsernamePasswordLoginStrategy implements LoginStrategy {
    ...
    public LoginResult login(LoginRequest request) {
        ...
        if (request.getUsername() == null || request.getUsername().trim().isEmpty()) {
            throw new BusinessException(CommonErrorCode.REQUIRED_PARAM_MISSING, "用户名不能为空");
        }
        ...
        // 根据用户名查找用户
        UserInfoDTO user = userAuthService.getUserByUsername(request.getTenantId(), request.getUsername());
        if (user == null) {
            throw new BusinessException(CommonErrorCode.USERNAME_OR_PASSWORD_ERROR, "用户名或密码错误");
        }
    ...
    public String getLoginType() {
        return LoginType.USERNAME_PASSWORD.getCode();
    }
