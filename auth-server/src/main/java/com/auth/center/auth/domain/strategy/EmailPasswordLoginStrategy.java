...
public class EmailPasswordLoginStrategy implements LoginStrategy {
    ...
    public LoginResult login(LoginRequest request) {
        ...
        if (request.getEmail() == null || request.getEmail().trim().isEmpty()) {
            throw new BusinessException(CommonErrorCode.REQUIRED_PARAM_MISSING, "邮箱不能为空");
        }
        ...
        // 根据邮箱查找用户
        UserInfoDTO user = userAuthService.getUserByEmail(request.getTenantId(), request.getEmail());
        if (user == null) {
            throw new BusinessException(CommonErrorCode.USERNAME_OR_PASSWORD_ERROR, "邮箱或密码错误");
        }
    ...
    public String getLoginType() {
        return LoginType.EMAIL_PASSWORD.getCode();
    }
