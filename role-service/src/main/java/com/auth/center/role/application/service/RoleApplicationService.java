...
public class RoleApplicationService {
    ...
    public SingleResponse<RoleDTO> updateRole(Long id, RoleDTO roleDTO) {
        ...
        // 系统角色不允许修改角色编码和角色类型
        if (existingEntity.getRoleType() == RoleType.SYSTEM) {
            if (!existingEntity.getRoleCode().equals(roleDTO.getRoleCode())) {
                throw new BusinessException(ErrorCode.SYSTEM_ROLE_CANNOT_MODIFY);
            }
            if (existingEntity.getRoleType() != roleDTO.getRoleType()) {
                throw new BusinessException(ErrorCode.SYSTEM_ROLE_CANNOT_MODIFY);
            }
        }
        ...
        existingEntity.setRoleType(roleDTO.getRoleType());
        existingEntity.setDataScope(roleDTO.getDataScope());
    ...
    public PageResponse<RoleDTO> getRolePage(Long tenantId, String roleName, RoleStatus status, 
        ...
        return PageResponse.of(roleDTOs, rolePage.getTotalElements(), page, size);
    }
    ...
    public Response enableRole(Long id) {
        ...
        roleEntity.setStatus(RoleStatus.ENABLED);
        roleEntity.setUpdatedTime(LocalDateTime.now());
    ...
    public Response disableRole(Long id) {
        ...
        roleEntity.setStatus(RoleStatus.DISABLED);
        roleEntity.setUpdatedTime(LocalDateTime.now());
        roleRepository.save(roleEntity);
    ...
    public Response deleteRole(Long id) {
        ...
        // 系统角色不允许删除
        if (roleEntity.getRoleType() == RoleType.SYSTEM) {
            throw new BusinessException(ErrorCode.SYSTEM_ROLE_CANNOT_DELETE);
        }
     ...
     * 获取用户拥有的角色列表
     */
    public List<RoleDTO> getUserRoles(Long tenantId, Long userId) {
