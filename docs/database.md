...
### 1.2 数据库选型
- **主数据库**: MySQL 8.0 (事务性操作)
- **缓存数据库**: Redis 7.0 (会话和缓存)
- **日志数据库**: Elasticsearch 7.x (日志检索)
...
### 3.2 分表策略
- **操作日志表**: 按月分表 (sys_operation_log_202401)
- **登录日志表**: 按月分表 (sys_login_log_202401)
- **大数据量表**: 按租户分表 (sys_user_tenant1)
...
### 4.1 数据隔离级别
- **数据库级别**: 不同租户使用不同数据库
- **Schema级别**: 同一数据库不同Schema
- **表级别**: 同一表通过tenant_id字段隔离

### 4.2 实现方案
- **MyBatis拦截器**: 自动添加tenant_id过滤条件
- **ThreadLocal**: 线程级别传递租户ID
- **数据源路由**: 动态数据源切换
